    ...
    if request.method == "POST":
        ...
        if check_user:
            ...
            # allow trainer to login
                ...
                cur.execute(
                    "SELECT trainer_id FROM trainer WHERE userid = %s;",
                    (check_user[0],),
                )
...
def register():
    ...
    if request.method == "POST":
        ...
        if existing_member:
            ...
            return render_template(
                "register.html", msg="A member with this email address already exists."
            )
...
def updatemember():
    ...
    if request.method == "POST":
        ...
        cur = getCursor()
        cur.execute(
            "UPDATE member SET firstname=%s, lastname=%s, mobile=%s, email=%s, street=%s, suburb=%s, city=%s, postalcode=%s, subscription_status=%s, join_date=%s where userid=%s;",
            (
                firstname,
                lastname,
                mobile,
        ...
        cur.execute(
            "select firstname, lastname, email, mobile, userid, street, suburb, city, postalcode, subscription_status, join_date from member where userid=%s;",
            (str(userid),),
        )
        ...
        return render_template(
            "search_results.html ", dbresult=select_result, dbcols=column_names
        )
    else:
        userid = request.form.get("userid")
        if userid == "":
            ...
            cur = getCursor()
            cur.execute("SELECT * FROM member where userid=%s;", (str(userid),))
...
def new_member():
    if request.method == "POST":
        ...
        if existing_member:
            ...
            return render_template(
                "new_member.html",
                msg="A member with this email address already exists.",
            )
...
Lincoln Fitness
        ...
        return render_template(
            "new_member.html",
            msg=f"{firstname} {lastname} has been successfully added to the system! The member should have received a confirmation email.",
        )
...
where trainer.trainer_id=%s and class_time >now() order by class_time;     
    ...
    return render_template(
        "trainer_home.html",
        specilized_course_trainer_classes=specilized_course_trainer_classes,
        group_course_trainer_classes=group_course_trainer_classes,
    )
...
def add_sessioncourse_member():
    if request.method == "GET":
        ...
        specilized_course_session_ID = request.args.get("specilized_course_sessionid")
        ...
        if group_course_session_ID != None:
            ...
            return render_template(
                "trainer_add_member_to_course.html",
                group_course_session_ID=group_course_session_ID,
                remaining_member_list=remaining_member_list,
            )
        ...
        if specilized_course_session_ID != None:
          ...
          from specializedclass_booking join specializedclass_session 
            ...
            return render_template(
                "trainer_add_member_to_course.html",
                specilized_course_session_ID=specilized_course_session_ID,
                remaining_member_list=remaining_member_list,
            )
        ...
        if group_course_session_ID != None:
            ...
            # insert data sql
            cur.execute(
                "INSERT INTO `groupclass_booking` (`session_id`, `member_id`) VALUES (%s, %s);",
            ...
            return render_template(
                "trainer_course_member_list.html",
                group_course_member_list=group_course_member_list,
                course_session_ID=group_course_session_ID,
            )
            ...
            # insert data sql
            cur.execute(
                "INSERT INTO `specializedclass_booking` (`specializedclass_sessionid`, `member_id`) VALUES (%s, %s);",
                (
            ...
            return render_template(
                "trainer_course_member_list.html",
                specilized_course_member_list=specilized_course_member_list,
                course_session_ID=specilized_course_session_id,
            )
...
def delete_sessioncourse_member():
    ...
    if group_course_session_ID != None:
        ...
        cur.execute(
            "select groupclass_bookingid from groupclass_booking where session_id=%s and member_id=%s;",
            (
                group_course_session_ID,
                member_id,
            ),
        )
        ...
        cur.execute(
            "DELETE FROM `groupclass_booking` WHERE (`groupclass_bookingid` = %s);",
            (booking_id,),
        )
        ...
        cur = getCursor()
        cur.execute(
            "select specializedclass_bookingid from specializedclass_booking where specializedclass_sessionid=%s and member_id=%s;",
            (
                specilized_course_session_id,
                member_id,
            ),
        ...
        # delete  booking id row
        cur.execute(
            "DELETE FROM `specializedclass_booking` WHERE (`specializedclass_bookingid` = %s);",
            (booking_id,),
        )
...
def traineredit(userid):
    if request.method == "GET":
        connection = getCursor()
        connection.execute("Select * from trainer WHERE userid = %s;", (userid,))
        trainerprofile = connection.fetchone()
        print(trainerprofile)
        return render_template(
            "trainer_modifyprofile.html", trainerprofile=trainerprofile, userid=userid
        )
...
def trainermodifyprofile():
    ...
    if request.method == "POST":
        ...
        cur.execute(
            "UPDATE trainer SET first_name=%s,last_name=%s,email=%s,mobile=%s,speciality=%s where userid=%s;",
            (
                first_name,
                last_name,
                email,
...
def trapro(member_id):
    ...
    if request.method == "GET":
        connection = getCursor()
        connection.execute("Select * from member WHERE member_id = %s;", (member_id,))
        member = connection.fetchone()
...
def updatemem():
    ...
    if request.method == "POST":
        ...
        cur = getCursor()
        cur.execute(
            "UPDATE member SET firstname = %s, lastname = %s, mobile = %s, email = %s, street = %s, suburb = %s, city = %s, postalcode = %s WHERE userid = %s;",
            (
                firstname,
                lastname,
                mobile,
...
def managerupdatemem():
    ...
    if request.method == "POST":
        ...
        return render_template(
            "search_members.html",
            message=f"{firstname} {lastname}'s details have been successfully updated!",
        )
...
def specialclass():
    connection = getCursor()
    sql = """select sp.classes_id, sp.class_type, sp.class_desc, sp.trainer_id, sp.schedule, t.first_name from specializedclasses sp 
...
def currentgroupbookings():
    ...
    return render_template(
        "currentgroupbookings.html", currentgroupbooking=currentgroupBooking
    )
...
@app.route(
    "/membersession/add/<int:specializedclass_sessionid>", methods=["GET", "POST"]
)
...
@app.route(
    "/payment_confirmation/<int:specializedclass_bookingid>", methods=["GET", "POST"]
)
def payment_confirmation(specializedclass_bookingid):
    connection = getCursor()
    userid = session["member_id"]
    connection.execute(
        "SELECT * from specializedclass_booking WHERE specializedclass_bookingid=%s;",
        (specializedclass_bookingid,),
    )
    booking = connection.fetchone()

    session_id = booking[2]
    connection.execute(
        "Select specializedclasses_id from specializedclass_session WHERE specializedclass_sessionid=%s;",
        (session_id,),
    )
    class_id = booking[1]
    connection.nextset()
    connection.execute(
        "SELECT sc.pricing FROM specializedclass_session s JOIN specializedclasses sc ON s.specializedclasses_id = sc.specializedclasses_id WHERE s.specializedclass_sessionid=%s;",
        (class_id,),
    )
    ...
    connection.execute(
        "INSERT INTO payment (member_id,payment_date,payment_amount,specializedclass_bookingid) VALUES (%s,%s,%s,%s);",
...
def monthreport():
    ...
    return render_template(
        "monthlyreport.html", payments=payments, total_payment=total_payment
    )
...
def sixmonthreport():
    ...
    total_payment = sum(payment[3] for payment in payments)
    return render_template(
        "sixmonthreport.html", payments=payments, total_payment=total_payment
    )
...
def annualreport():
    ...
    total_payment = sum(payment[3] for payment in payments)
    return render_template(
        "annualreport.html", payments=payments, total_payment=total_payment
    )
...
def contact():
    if request.method == "POST":
        ...
        connection = getCursor()
        connection.execute(
            "Insert into contact(name, mobile, email, message) VALUES (%s,%s,%s,%s);",
            (
                name,
                mobile,
                email,
